...
/*   Updated: 2024/05/13 17:53:50 by abquaoub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_command(t_data *data, t_list *head, char *cmd, char **command)
{
	int	flag;
...
void	ft_exec_command(t_data *data, t_list *head)
{
	char	**command;
	char	*cmd;
	...
	if (cmd[0])
	{
		...
		else
		{
			...
			exec_command(data, head, cmd, command);
		}
	}
	ft_free(command);
	free(cmd);
...
void	ft_command(t_list *head, t_data *data)
{
	...
	ft_exec_command(data, head->command);
}
